...
from custom_dueling_dqn import CustomDuelingDQNPolicy
from ppo_evaluate import latest_observation_dict, scalar
from run_hla2010_dqn_discrete_action_probe_012_01 import ACTION_TABLE, IRRIGATION_WINDOWS, NITROGEN_WINDOWS
from run_hla2010_dqn_economic_reward_probe_012_03 import harvest_yields_from_mgmt, make_env
from run_hla_official_reward_restart_smoke import install_official_reward_module, parse_events, prepare_case_at


OUT_DIR = PROJECT_ROOT / "DSSAT_auto_validation" / "HLA_2004" / "hla2015_dueling_dqn_economic_012_14"
...
def train_and_eval(year: int, timesteps: int, seed: int, water_cost: float, nitrogen_cost: float, label: str) -> Path:
    ...
    debug_log = case_dir / "dueling_dqn_economic_debug.log"
    ...
    try:
        ...
        log("learn:done")
        model.save(str(model_dir / "dueling_dqn_economic_reward_probe"))
    finally:
        env.close()
    ...
    daily.to_csv(case_dir / "dueling_dqn_economic_eval_daily.csv", index=False, encoding="utf-8-sig")
    events = parse_events(snapshot / "MgmtEvent.OUT")
    hvals = harvest_yields_from_mgmt(snapshot / "MgmtEvent.OUT")
    summary = {
        "algorithm": "CustomDuelingDQN",
        ...
        "dueling_dqn_harvest_yield_kg_ha": hvals[-1] if hvals else None,
...
def main() -> None:
    ...
    parser.add_argument("--label", default="dueling_dqn_medium_N_cost")
