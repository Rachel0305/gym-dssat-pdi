"""055_02: YCA/YC lowIC four-baseline rebuild with static level-1 correction.
...
DEFAULT_CONFIG = ROOT / "configs" / "055_02_yca_lowIC_four_baselines_static_level1.json"
...
def read_config(path: Path) -> dict[str, Any]:
    ...
    if str(cfg.get("station_code")) != "YCA":
        raise ValueError("055_02 is registered for station_code=YCA.")
    if str(cfg.get("site")) != "YC":
        raise ValueError("055_02 is registered for DSSAT site=YC.")
...
def write_record(out: Path, cfg: dict[str, Any], pf: dict[str, Any], summary: pd.DataFrame, manifest: pd.DataFrame, audit: pd.DataFrame, elapsed: float) -> Path:
    ...
    lines = [
        ...
        f"- Summary CSV: `{rel(out / 'evaluation' / '055_02_baseline_summary.csv')}`.",
        f"- Event audit CSV: `{rel(out / 'evaluation' / '055_02_management_event_audit.csv')}`.",
        "",
        "## Scenario means",
        "",
...
def run(cfg_path: Path, dry_run: bool, years_override: list[int] | None = None, run_id: str = "") -> dict[str, Any]:
    ...
    if dry_run:
        return {"task": "055_02_yca_static_level1_four_baselines", "mode": "dry_run", **pf, "output_root": rel(out)}
    ...
    selected.to_csv(out / "configs" / "055_02_selected_years.csv", index=False, encoding="utf-8-sig")
    run_config, env_config = build_configs(out, selected)
    direct_ppo.write_yaml(env_config, out / "configs" / "055_02_resolved_env_config.yaml")
    ...
    try:
        for row in selected.itertuples(index=False):
            for scenario in SCENARIOS:
                print(f"[055_02] YCA{int(row.year)} {scenario}", flush=True)
                try:
                    ...
                    summary["source_status"] = "generated_055_02_static_level1_corrected_baseline"
    ...
    summary.to_csv(out / "evaluation" / "055_02_baseline_summary.csv", index=False, encoding="utf-8-sig")
    daily.to_csv(out / "evaluation" / "055_02_baseline_daily.csv", index=False, encoding="utf-8-sig")
    manifest.to_csv(out / "evaluation" / "055_02_coverage_manifest.csv", index=False, encoding="utf-8-sig")
    audit.to_csv(out / "evaluation" / "055_02_management_event_audit.csv", index=False, encoding="utf-8-sig")
    (out / "055_02_preflight.json").write_text(json.dumps(pf, indent=2, ensure_ascii=False), encoding="utf-8")
    record = write_record(out, cfg, pf, summary, manifest, audit, time.time() - started)
    result = {"task": "055_02_yca_static_level1_four_baselines", "output_root": rel(out), "record_md": rel(record), "successful_runs": int(len(summary)), "non_ok_runs": int((manifest["status"] != "ok").sum()) if not manifest.empty else 0}
    (out / "055_02_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    return result
