...
def missing_row(site: str, agent: str, source: str) -> dict:
    ...
    return {
        ...
        "turfac_stress_days_gt_0.10": np.nan,
        "max_turfac": np.nan,
        "mean_turfac": np.nan,
...
def main() -> None:
    ...
    md = [
        ...
        "",
        "`swfac` and `nstres` are exposed in maize `all` mode. `turfac` is kept as a column and will be saved if a safe environment exposes it. In this installed environment maize stress variables are post-processed as `1 - original`, so larger values indicate stronger stress.",
        "",
