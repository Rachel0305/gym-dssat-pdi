...
CONFIG = ROOT / "experiments" / "ppo_observed_years" / "config_031_18_free_timing_discrete_maskableppo_ncost2x_scaled_reward_seeds1_2.yaml"
OUT = ROOT / "benchmark_results" / "031_18_free_timing_discrete_maskableppo_ncost2x_scaled_reward_seeds1_2"
DOC = ROOT / "docs" / "031_18_free_timing_discrete_maskableppo_ncost2x_scaled_reward_seeds1_2_record.md"
...
def make_sy2014_selection() -> pd.DataFrame:
    ...
    row["selection_reason"] = "031_18_free_timing_discrete_maskableppo_ncost2x_scaled_reward_seeds1_2"
    return row
...
class FreeTimingDiscreteMaskablePPOWrapper(gym.Env):
...
def make_env(config: dict, env_config: dict, station: str, year: int, seed: int, run_tag: str, evaluation: bool = False):
    ...
    return FreeTimingDiscreteMaskablePPOWrapper(base, config)
...
def ppo_kwargs(config: dict) -> dict[str, Any]:
    ...
    return {
        ...
        "policy_kwargs": {"net_arch": [int(x) for x in cfg["net_arch"]]},
    }


def train_one_seed(config: dict, env_config: dict, seed: int) -> pd.DataFrame:
    from sb3_contrib import MaskablePPO

    station = "SYA"
    year = 2014
    model_path = OUT / "models" / station / f"free_timing_discrete_maskableppo_ncost2x_scaled_reward_seed{seed}.zip"
    env = None
    status = "ok"
    notes = ""
    try:
        env = make_env(config, env_config, station, year, seed, f"{station}_{year}_031_18_train", evaluation=False)
        model = MaskablePPO(
            "MlpPolicy",
            env,
            verbose=0,
...
def train(config: dict, env_config: dict) -> pd.DataFrame:
    rows = [train_one_seed(config, env_config, seed) for seed in [1, 2]]
    summary = pd.concat(rows, ignore_index=True)
    summary.to_csv(OUT / "evaluation" / "training_run_summary.csv", index=False, encoding="utf-8-sig")
    return summary


def summarize_daily(daily: pd.DataFrame, daily_path: Path, split: str, model_path: Path, seed: int) -> dict[str, Any]:
    ...
    return {
        "algorithm": "free_timing_discrete_MaskablePPO",
        "policy_name": f"free_discrete_maskableppo_ncost2x_scaled_reward_20k_seed{seed}",
        "split": split,
...
def evaluate(config: dict, env_config: dict, train_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in train_summary.iterrows():
        if str(row["run_status"]) != "ok":
            rows.append({"run_status": "failed", "notes": row.get("notes", ""), "seed": int(row.get("seed", -1))})
            continue
        seed = int(row["seed"])
        model_path = ROOT / str(row["model_path"])
        model = MaskablePPO.load(str(model_path), device="cpu")
        for split in ["train", "eval"]:
            env = make_env(config, env_config, station, year, seed, f"{station}_{year}_031_18_seed{seed}_{split}", evaluation=True)
            ...
            daily_path = OUT / "daily_outputs" / station / f"2014_seed{seed}_{split}_free_discrete_maskableppo_ncost2x_scaled_reward_daily.csv"
            daily.to_csv(daily_path, index=False, encoding="utf-8-sig")
            row_out = summarize_daily(daily, daily_path, split, model_path, seed)
...
def write_record(train_summary: pd.DataFrame, eval_summary: pd.DataFrame, comparison: pd.DataFrame) -> None:
    lines = [
        "# 031_18 Free-timing discrete MaskablePPO ncost2x scaled-reward seeds1_2 record",
        "",
        "## Scope",
        "",
        "- SYA2014 seeds 1 and 2 only.",
        ...
        "",
        "This is an exact cross-seed repeat of the 031_17 scaled-reward configuration for seeds 1 and 2. It tests whether the seed0 improvement was reproducible.",
        "",
        "Branch rules:",
        "",
        "- If seed1 and seed2 both reproduce high-yield, reduced-input behavior, the scaled-reward discrete MaskablePPO branch can move to broader site-year testing.",
        "- If only one of seed1/seed2 reproduces the improvement, mark the branch as seed-sensitive and do not expand before a stability decision.",
        "- If neither seed reproduces the improvement, freeze this branch as a seed0-only positive.",
    ]
    DOC.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    (OUT / "031_18_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
