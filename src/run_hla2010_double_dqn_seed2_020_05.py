...
OUT_ROOT = ROOT / "DSSAT_auto_validation" / "HLA_2004" / "hla2010_double_dqn_seed2_020_05"
...
def write_record(run_dir: Path, timesteps: int, interval: int, summary: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "- Seed: 2。",
        f"- Timesteps: {timesteps}；checkpoint interval: {interval}。",
        ...
        "## 固定规则选点",
        "",
        f"- Step: {int(chosen['checkpoint_step'])}。",
        f"- Yield: {chosen['final_grain_kg_ha']:.1f} kg/ha。",
        f"- Irrigation: {chosen['action_irrigation_total']:.1f} mm。",
        f"- Nitrogen: {chosen['action_fertilizer_total']:.1f} kg/ha。",
        f"- Reward: {chosen['total_reward']:.1f}。",
        "",
...
def main() -> None:
    ...
    run_dir = OUT_ROOT / f"double_dqn_seed2_{args.timesteps}steps"
    if run_dir.exists():
    ...
    daily.to_csv(run_dir / "double_dqn_eval_daily.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(run_dir / "checkpoint_summary.csv", index=False, encoding="utf-8-sig")
    figure = run_dir / "figures" / f"hla2010_double_dqn_seed2_{args.timesteps}steps.png"
    base.plot_checkpoints(daily, summary, figure, 2010, 2, args.timesteps)
    write_record(run_dir, args.timesteps, args.checkpoint_interval, summary)
