...
OUT_DIR = PROJECT_ROOT / "DSSAT_auto_validation" / "multisite_new_cultivar_yc2014_four_scenario_smoke_013_03"
DOC_PATH = PROJECT_ROOT / "docs" / "2026-06-30_013_03_yc2014_four_scenario_smoke_record.md"
...
MIN_INTERVAL_DAYS = 7
IRRIGATION_WINDOWS = [(20, 35), (40, 60), (65, 90)]
NITROGEN_WINDOWS = [(1, 10), (35, 55), (60, 80)]

SCENARIO_ORDER = ["null", "recorded", "dssat_auto", "dqn_economic_smoke"]
SCENARIO_LABELS = {
    "null": "Null",
    "recorded": "Recorded",
    "dssat_auto": "DSSAT auto",
    "dqn_economic_smoke": "DQN economic smoke",
}
SCENARIO_COLORS = {
    "null": "#464C55",
    "recorded": "#CC6F47",
    "dssat_auto": "#5477C4",
    "dqn_economic_smoke": "#386411",
}
...
class YCDiscreteBudgetedWrapper(gymnasium_base.Env):
    ...
    def _safe_real_action(self, raw_real: dict[str, float], dap: int) -> dict[str, float]:
        ...
        if not can_operate:
            return {"amir": 0.0, "anfer": 0.0}
        in_irrigation_window = any(left <= dap <= right for left, right in IRRIGATION_WINDOWS)
        in_nitrogen_window = any(left <= dap <= right for left, right in NITROGEN_WINDOWS)
...
def prepare_case_for_scenario(scenario: str) -> Path:
    ...
    text = prepare_text_for_scenario(source, TRNO, scenario if scenario != "dqn_economic_smoke" else "null")
...
def run_dqn_smoke() -> tuple[pd.DataFrame, dict[str, Any]]:
    print("[dqn] prepare", flush=True)
    run_dir = prepare_case_for_scenario("dqn_economic_smoke")
    env_args = json.loads((run_dir / "env_args.json").read_text(encoding="utf-8"))
    from stable_baselines3 import DQN

    print("[dqn] train env init", flush=True)
    train_env = EconomicRewardWrapper(YCDiscreteBudgetedWrapper(make_raw_env(env_args)))
    model = DQN(
        "MlpPolicy",
        train_env,
        verbose=0,
    ...
    eval_env = EconomicRewardWrapper(YCDiscreteBudgetedWrapper(make_raw_env(env_args)))
    rows = []
    try:
    ...
    daily = pd.DataFrame(rows)
    events = parse_events_eval(run_dir, "dqn_economic_smoke")
    plantgro = parse_dssat_table(run_dir / "pdi_tmp_snapshot_eval" / "PlantGro.OUT")
    summary = {
        "scenario": "dqn_economic_smoke",
        "event_irrigation_total": float(events.loc[events["unit"].eq("mm"), "amount"].sum()) if not events.empty else float(daily["irrigation_mm"].sum()),
        "event_fertilizer_total": float(events.loc[events["unit"].str.contains("kg", na=False), "amount"].sum()) if not events.empty else float(daily["fertilizer_kg_ha"].sum()),
        "final_grain_kg_ha": float(plantgro["GWAD"].dropna().iloc[-1]) if "GWAD" in plantgro.columns else np.nan,
        "final_biomass_kg_ha": float(plantgro["CWAD"].dropna().iloc[-1]) if "CWAD" in plantgro.columns else np.nan,
        "max_water_stress": float(daily["swfac"].max()) if not daily.empty else np.nan,
        "max_nitrogen_stress": float(daily["nstres"].max()) if not daily.empty else np.nan,
    ...
    daily.to_csv(run_dir / "dqn_smoke_eval_daily.csv", index=False, encoding="utf-8-sig")
    return daily, summary
...
def process_plot(data: pd.DataFrame, out_path: Path) -> None:
    ...
    axes[1].set_title("YC 2014 four-scenario smoke process plot", loc="left", fontsize=14)
...
def main() -> None:
    ...
    parser.add_argument("--only", choices=["null", "recorded", "dssat_auto", "dqn"], default=None)
    ...
    all_daily = []
    summaries = []
    if args.only in (None, "null", "recorded", "dssat_auto"):
        scenarios = [args.only] if args.only in {"null", "recorded", "dssat_auto"} else ["null", "recorded", "dssat_auto"]
        for scenario in scenarios:
            daily, summary = run_zero_action_scenario(scenario)
            all_daily.append(daily)
            summaries.append(summary)
    if args.only in (None, "dqn"):
        dqn_daily, dqn_summary = run_dqn_smoke()
    ...
    all_daily_df.to_csv(OUT_DIR / "013_03_yc2014_four_scenario_daily.csv", index=False, encoding="utf-8-sig")
    summary_df.to_csv(OUT_DIR / "013_03_yc2014_four_scenario_summary.csv", index=False, encoding="utf-8-sig")
    process_plot(all_daily_df, figures_dir / "yc2014_four_scenario_smoke_process.png")
