...
CASE_DIR = (
    ...
    / "2015"
    / "literature_nstep_nstep5_seed0_5000steps"
)
OUT_DIR = ROOT / "hla2015_literature_nstep_q_value_diagnostic_012_18"
...
def run_q_diagnostic() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    model_path = CASE_DIR / "models" / "dqn_literature_nstep_probe.zip"
...
def summarize(qdf: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = {
        "run": "012_17_literature_nstep5_seed0_5k",
        "steps": int(len(qdf)),
...
def main() -> None:
    ...
    qdf.to_csv(OUT_DIR / "012_17_nstep5_seed0_5k_q_values_daily.csv", index=False, encoding="utf-8-sig")
    topdf.to_csv(OUT_DIR / "012_17_nstep5_seed0_5k_top8_actions_daily.csv", index=False, encoding="utf-8-sig")
    key = topdf[topdf["dap_before"].round().isin([21, 28, 35, 46, 53, 60, 67, 74, 81, 88, 95])]
    key.to_csv(OUT_DIR / "012_17_nstep5_seed0_5k_top8_actions_key_daps.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(OUT_DIR / "012_17_nstep5_seed0_5k_q_value_summary.csv", index=False, encoding="utf-8-sig")
    (OUT_DIR / "012_17_nstep5_seed0_5k_q_value_summary.json").write_text(
