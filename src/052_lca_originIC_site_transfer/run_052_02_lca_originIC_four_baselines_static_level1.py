"""052_02: LCA/LC originIC four-baseline rebuild with static level-1 correction.
...
DEFAULT_CONFIG = ROOT / "configs" / "052_02_lca_originIC_four_baselines_static_level1.json"
...
def read_config(path: Path) -> dict[str, Any]:
    ...
    if str(cfg.get("station_code")) != "LCA":
        raise ValueError("052_02 is registered for station_code=LCA.")
    if str(cfg.get("site")) != "LC":
        raise ValueError("052_02 is registered for DSSAT site=LC.")
...
def preflight(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    mzx = input_root / "LC" / "CNLC0801.MZX"
    schedule = recorded_template_schedules_dedup().get("LC", {})
    issues = []
    if not input_root.exists():
        issues.append("input root missing")
    if not mzx.exists():
        issues.append("CNLC0801.MZX missing")
    return {
        "station_code": "LCA",
...
def recorded_template_schedules_dedup() -> dict[str, dict[int, dict[str, float]]]:
    ...
    The old 027_05 daily table stores the same recorded LC2014 line under both
...
def selected_rows(years: list[int]) -> pd.DataFrame:
    ...
    out = split[split["station_code"].astype(str).eq("LCA") & split["year"].isin(years)].copy()
...
def write_record(out: Path, cfg: dict[str, Any], pf: dict[str, Any], summary: pd.DataFrame, manifest: pd.DataFrame, audit: pd.DataFrame, elapsed: float) -> Path:
    ...
    lines = [
        f"# {cfg['task_id']} LCA/LC {cfg['input_profile']} four-baseline static level-1 record",
        "",
        ...
        f"- Summary CSV: `{rel(out / 'evaluation' / '052_02_baseline_summary.csv')}`.",
        f"- Event audit CSV: `{rel(out / 'evaluation' / '052_02_management_event_audit.csv')}`.",
        "",
        "## Scenario means",
        "",
...
def run(cfg_path: Path, dry_run: bool, years_override: list[int] | None = None, run_id: str = "") -> dict[str, Any]:
    ...
    if dry_run:
        return {"task": "052_02_lca_static_level1_four_baselines", "mode": "dry_run", **pf, "output_root": rel(out)}
    ...
    selected.to_csv(out / "configs" / "052_02_selected_years.csv", index=False, encoding="utf-8-sig")
    run_config, env_config = build_configs(out, selected)
    direct_ppo.write_yaml(env_config, out / "configs" / "052_02_resolved_env_config.yaml")
    ...
    try:
        for row in selected.itertuples(index=False):
            for scenario in SCENARIOS:
                print(f"[052_02] LCA{int(row.year)} {scenario}", flush=True)
                try:
                    daily, summary, checks = baseline.evaluate_scenario(run_config, env_config, "LCA", int(row.year), scenario, schedules)
                    summary["season_last_dap"] = int(pd.to_numeric(daily["dap"], errors="coerce").max())
                    summary["source_input_profile"] = cfg["input_profile"]
                    summary["source_status"] = "generated_052_02_static_level1_corrected_baseline"
                    summary["render_check_count"] = len(checks)
                    daily["source_input_profile"] = cfg["input_profile"]
                    daily_frames.append(daily)
                    summary_rows.append(summary)
                    schedule = {} if scenario == "dssat_auto" else baseline.schedule_for(scenario, "LC", schedules)
    ...
    summary.to_csv(out / "evaluation" / "052_02_baseline_summary.csv", index=False, encoding="utf-8-sig")
    daily.to_csv(out / "evaluation" / "052_02_baseline_daily.csv", index=False, encoding="utf-8-sig")
    manifest.to_csv(out / "evaluation" / "052_02_coverage_manifest.csv", index=False, encoding="utf-8-sig")
    audit.to_csv(out / "evaluation" / "052_02_management_event_audit.csv", index=False, encoding="utf-8-sig")
    (out / "052_02_preflight.json").write_text(json.dumps(pf, indent=2, ensure_ascii=False), encoding="utf-8")
    record = write_record(out, cfg, pf, summary, manifest, audit, time.time() - started)
    result = {"task": "052_02_lca_static_level1_four_baselines", "output_root": rel(out), "record_md": rel(record), "successful_runs": int(len(summary)), "non_ok_runs": int((manifest["status"] != "ok").sum()) if not manifest.empty else 0}
    (out / "052_02_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    return result
