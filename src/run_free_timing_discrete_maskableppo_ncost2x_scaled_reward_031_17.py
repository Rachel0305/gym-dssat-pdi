...
CONFIG = ROOT / "experiments" / "ppo_observed_years" / "config_031_17_free_timing_discrete_maskableppo_ncost2x_scaled_reward_seed0.yaml"
OUT = ROOT / "benchmark_results" / "031_17_free_timing_discrete_maskableppo_ncost2x_scaled_reward_seed0"
DOC = ROOT / "docs" / "031_17_free_timing_discrete_maskableppo_ncost2x_scaled_reward_seed0_record.md"
...
def make_sy2014_selection() -> pd.DataFrame:
    ...
    row["selection_reason"] = "031_17_free_timing_discrete_maskableppo_ncost2x_scaled_reward_seed0"
    return row
...
def train(config: dict, env_config: dict) -> pd.DataFrame:
    ...
    model_path = OUT / "models" / station / "free_timing_discrete_maskableppo_ncost2x_scaled_reward_seed0.zip"
    env = None
    status = "ok"
    notes = ""
    try:
        env = make_env(config, env_config, station, year, seed, f"{station}_{year}_031_17_train", evaluation=False)
        model = MaskablePPO(
            "MlpPolicy",
            env,
            verbose=1,
...
def summarize_daily(daily: pd.DataFrame, daily_path: Path, split: str, model_path: Path) -> dict[str, Any]:
    ...
    return {
        "algorithm": "free_timing_discrete_MaskablePPO",
        "policy_name": "free_discrete_maskableppo_ncost2x_scaled_reward_20k_seed0",
        "split": split,
...
def evaluate(config: dict, env_config: dict, train_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    for split in ["train", "eval"]:
        env = make_env(config, env_config, station, year, seed, f"{station}_{year}_031_17_{split}", evaluation=True)
        ...
        daily_path = OUT / "daily_outputs" / station / f"2014_{split}_free_discrete_maskableppo_ncost2x_scaled_reward_daily.csv"
...
def write_record(train_summary: pd.DataFrame, eval_summary: pd.DataFrame, comparison: pd.DataFrame) -> None:
    lines = [
        "# 031_17 Free-timing discrete MaskablePPO ncost2x scaled-reward seed0 record",
        "",
        "## Scope",
        "",
        "- SYA2014 seed0 only.",
...
def main() -> None:
    ...
    (OUT / "031_17_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
