"""049_00: HLA originIC expanded-action MaskablePPO site transfer.
...
def read_config(path: Path) -> dict[str, Any]:
    ...
    if cfg.get("station_code") != "HLA" or cfg.get("site") != "HLA":
        raise ValueError("049_00 is registered for station_code=HLA and site=HLA.")
    ...
    if irrigation != EXPECTED_IRRIGATION_LEVELS or nitrogen != EXPECTED_NITROGEN_LEVELS:
        raise ValueError("049_00 keeps the 046_10 action grid: I [0,15,30,45], N [0,40,80,120].")
    obs = cfg.get("observation_contract", {})
    if obs.get("base") != "046_02_raw_observation":
        raise ValueError("049_00 must inherit the 046_02 raw observation contract.")
    if bool(obs.get("normalization_enabled", True)) or bool(obs.get("weather_forecast_enabled", True)):
        raise ValueError("049_00 forbids observation normalization and weather forecast features.")
    if cfg.get("reference_run") != "046_10_sya_originIC_expanded_action_maskableppo":
        raise ValueError("049_00 must reference the frozen SYA 046_10 method.")
    return cfg
...
def preflight(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    source_mzx = input_root / "HL" / "CNHL0701_corrected_IC123.MZX"
    ...
    if not source_mzx.exists():
        issues.append(f"HLA source MZX missing: {rel(source_mzx)}")
...
def write_record(
    ...
    lines = [
        f"# {cfg['task_id']} HLA originIC expanded-action MaskablePPO record",
        "",
        "## Design",
        "",
...
def run(cfg_path: Path, dry_run: bool, smoke: bool, formal: bool) -> dict[str, Any]:
    ...
    if formal:
        ...
        if not smoke_verification["next_step_allowed"]:
            pf["issues"].append("049_00 completed smoke verification did not pass")
