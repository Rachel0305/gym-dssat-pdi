...
OUT_DIR = ROOT / "hla2015_literature_dqn_q_value_diagnostic_012_16"
...
def run_q_diagnostic() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    model_path = CASE_DIR / "models" / "dqn_literature_aligned_probe.zip"
    ...
    summary = summarize(qdf, topdf)
    return qdf, topdf, summary


def summarize(qdf: pd.DataFrame, topdf: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = {
        "run": "012_15_literature_aligned_seed0_5k",
        "steps": int(len(qdf)),
    ...
    # Top-1 frequency by raw action label inside operation windows.
...
def main() -> None:
    ...
    qdf.to_csv(OUT_DIR / "012_15_seed0_5k_q_values_daily.csv", index=False, encoding="utf-8-sig")
    topdf.to_csv(OUT_DIR / "012_15_seed0_5k_top8_actions_daily.csv", index=False, encoding="utf-8-sig")
    key = topdf[topdf["dap_before"].round().isin([27, 35, 46, 53, 60, 67, 74, 82, 89, 96])]
    key.to_csv(OUT_DIR / "012_15_seed0_5k_top8_actions_key_daps.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(OUT_DIR / "012_15_seed0_5k_q_value_summary.csv", index=False, encoding="utf-8-sig")
    (OUT_DIR / "012_15_seed0_5k_q_value_summary.json").write_text(
