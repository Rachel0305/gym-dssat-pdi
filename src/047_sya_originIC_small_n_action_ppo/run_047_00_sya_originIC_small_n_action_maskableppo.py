"""047_00: isolated originIC small-N 16-action MaskablePPO experiment.

This runner preserves the completed 046_10 originIC expanded-action experiment
except for one controlled change: the nitrogen grid is reduced from
[0, 40, 80, 120] to [0, 10, 20, 40]. Teacher guidance is intentionally disabled.
"""
...
DEFAULT_CONFIG = TASK_DIR / "config_047_00_sya_originIC_small_n_action_maskableppo.json"
PROMPT = ROOT / "prompts" / "047_00_sya_originIC_small_n_action_maskableppo.md"
SMOKE_TIMESTEPS = 2_000
SMOKE_CHECKPOINTS = [1_000, 2_000]
EXPECTED_IRRIGATION_LEVELS = [0.0, 15.0, 30.0, 45.0]
EXPECTED_NITROGEN_LEVELS = [0.0, 10.0, 20.0, 40.0]
...
def read_config(path: Path) -> dict[str, Any]:
    ...
    if irrigation != EXPECTED_IRRIGATION_LEVELS or nitrogen != EXPECTED_NITROGEN_LEVELS:
        raise ValueError(
            "047_00 action contract requires irrigation [0,15,30,45] and nitrogen [0,10,20,40]."
        )
    obs = cfg.get("observation_contract", {})
    if obs.get("base") != "046_02_raw_observation":
        raise ValueError("047_00 must inherit the 046_02 raw observation.")
    if bool(obs.get("normalization_enabled", True)) or bool(obs.get("weather_forecast_enabled", True)):
        raise ValueError("047_00 forbids observation normalization and weather forecast features.")
    if cfg.get("reference_run") != "046_10_sya_originIC_expanded_action_maskableppo":
        raise ValueError("047_00 must reference the 046_10 expanded-action PPO baseline.")
    teacher = cfg.get("teacher_guidance", {})
    if bool(teacher.get("enabled", False)):
        raise ValueError("047_00 is action-space-only; teacher guidance must be disabled.")
    return cfg
...
def audit_actions(out: Path, cfg: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for record in summary.itertuples(index=False):
        ...
        novel = irrigation.isin([15.0, 30.0]) | nitrogen.isin([10.0, 20.0])
    ...
    checks.update(
        {
            ...
            "smoke_teacher_guidance_disabled": not bool(smoke_config.get("teacher_guidance", {}).get("enabled", False)),
