...
import run_sya_lowIC_binary_timing_forecast_normalized_maskableppo_043_02 as base04302
...
TASK_ID = "043_05"
TASK_NAME = "sya_lowIC_binary_forecast_teacher_warmstart_maskableppo"
...
def ensure_dirs(out: Path) -> None:
    for rel in ["configs", "models/SYA", "evaluation", "daily_outputs/SYA", "logs", "bc_dataset", "audits"]:
...
def build_config_and_env_config(years: list[int]) -> tuple[dict[str, Any], dict[str, Any]]:
    config = base04302.load_config()
    selection = base04100.build_selection_for_years(years)
    env_config = base04302.direct_ppo.build_env_config(config, selection)
...
def action_index_for(env: Any, amir: float, anfer: float) -> int:
    ...
    if len(matches) != 1:
        raise RuntimeError(f"Cannot map action amir={amir}, anfer={anfer}; matches={matches}; grid={env.grid}")
...
def binary_teacher_action(requested: dict[str, float]) -> dict[str, float]:
    return {
        "amir": 45.0 if float(requested.get("amir", 0.0)) > 0 else 0.0,
        "anfer": 80.0 if float(requested.get("anfer", 0.0)) > 0 else 0.0,
    }


def make_env(config: dict[str, Any], env_config: dict[str, Any], year: int, run_tag: str, evaluation: bool):
    return base04302.make_env_04302(config, env_config, STATION, int(year), SEED, run_tag, evaluation=evaluation)
...
def collect_bc_dataset(out: Path, selected: pd.DataFrame, config: dict[str, Any], env_config: dict[str, Any]) -> tuple[dict[str, np.ndarray], pd.DataFrame]:
    ...
    ppo_safe_rendering.MULTISITE_INPUT_ROOT = base04302.LOWIC_INPUT_ROOT
    try:
        for teacher in selected.itertuples(index=False):
            year = int(teacher.year)
            schedule = combined_schedule(teacher)
            env = make_env(config, env_config, year, f"{STATION}_{year}_043_05_bc_collect", evaluation=True)
            fired: set[int] = set()
            try:
                obs, info = env.reset()
                for step in range(420):
                    ...
                    if dap in schedule:
                        fired.add(dap)
                    requested = binary_teacher_action(requested_raw)
                    ...
                    if not bool(mask[action_idx]):
                        skipped_rows.append(
                            {
                                ...
                                "step": int(step),
                                "dap": int(dap),
                                "teacher_tier": str(teacher.teacher_tier),
                        ...
                        if not bool(mask[action_idx]):
                            raise RuntimeError(
                                f"No-op also masked after skipping teacher action: year={year}, step={step}, dap={dap}"
                            )
    ...
    class_count = max(len(action_counts), 1)
    class_weights = {int(action): len(action_rows) / (class_count * count) for action, count in action_counts.items()}
    balanced_weights = np.asarray(
        [float(weight) * float(class_weights[int(action)]) for weight, action in zip(weight_rows, action_rows)],
        dtype=np.float32,
    )
    balanced_weights = balanced_weights / max(float(np.mean(balanced_weights)), 1e-9)
    if len(meta) > 0:
        meta["action_class_count"] = [int(action_counts[int(action)]) for action in action_rows]
        meta["action_class_balance_weight"] = [float(class_weights[int(action)]) for action in action_rows]
        meta["final_sample_weight"] = balanced_weights.astype(float)
    ...
    if len(meta) == 0:
        raise RuntimeError("No BC samples collected after teacher-action masking checks.")
    np.savez_compressed(out / "bc_dataset" / "043_05_bc_dataset.npz", **dataset)
    meta.to_csv(out / "bc_dataset" / "043_05_bc_dataset_meta.csv", index=False, encoding="utf-8-sig")
    skipped.to_csv(out / "bc_dataset" / "043_05_bc_skipped_masked_teacher_actions.csv", index=False, encoding="utf-8-sig")
    return dataset, meta
...
class RandomYearEnv(gym.Env):
    ...
    def _env_for(self, year: int):
        ...
        if year not in self.envs:
            self.envs[year] = make_env(self.config, self.env_config, year, f"{STATION}_{year}_043_05_train", evaluation=False)
...
class FixedStepCheckpointCallback:
    ...
    def model_path(self, step: int) -> Path:
        return self.out / "models" / STATION / f"{STATION}_binary_forecast_teacher_warmstart_seed{SEED}_ckpt{int(step)}.zip"
    ...
    for epoch in range(1, BC_EPOCHS + 1):
        ...
        for start in range(0, n, BC_BATCH_SIZE):
            ...
            with torch.no_grad():
                ...
                batch_nonzero = batch_actions != 0
                if bool(batch_nonzero.any()):
                    nonzero_acc = (pred[batch_nonzero] == batch_actions[batch_nonzero]).float().mean()
                    nonzero_accuracies.append(float(nonzero_acc.detach().cpu()))
            losses.append(float(loss.detach().cpu()))
            accuracies.append(float(acc.detach().cpu()))
        with torch.no_grad():
            try:
                full_distribution = model.policy.get_distribution(obs, action_masks=masks)
            except TypeError:
                full_distribution = model.policy.get_distribution(obs)
            full_probs = full_distribution.distribution.probs
            full_pred = torch.argmax(full_probs.masked_fill(~masks, -1.0), dim=1)
            full_nonzero_acc = (
                (full_pred[nonzero_actions] == actions[nonzero_actions]).float().mean()
                if bool(nonzero_actions.any())
                else torch.as_tensor(float("nan"), device=device)
            )
            full_nonzero_pred_rate = (full_pred != 0).float().mean()
        rows.append(
            {
                "bc_epoch": epoch,
                "bc_loss": float(np.mean(losses)),
                "bc_action_accuracy": float(np.mean(accuracies)),
                "bc_nonzero_batch_action_accuracy": float(np.mean(nonzero_accuracies)) if nonzero_accuracies else float("nan"),
                "bc_full_nonzero_action_accuracy": float(full_nonzero_acc.detach().cpu()),
                "bc_full_nonzero_pred_rate": float(full_nonzero_pred_rate.detach().cpu()),
            }
        )
        if not np.isfinite(rows[-1]["bc_loss"]):
            raise RuntimeError(f"BC loss is not finite at epoch {epoch}")
    bc_log = pd.DataFrame(rows)
    bc_log.to_csv(out / "logs" / "043_05_bc_pretrain_log.csv", index=False, encoding="utf-8-sig")
    return bc_log


def evaluate_checkpoint(out: Path, model_path: Path, checkpoint_step: int, stage: str, config: dict[str, Any], env_config: dict[str, Any], thresholds: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        for year in YEARS:
            env = make_env(config, env_config, year, f"{STATION}_{year}_043_05_ckpt{checkpoint_step}_eval", evaluation=True)
            daily_rows = []
            try:
                obs, info = env.reset()
                for step in range(420):
                ...
                daily_path = out / "daily_outputs" / STATION / f"043_05_{STATION}_{year}_ckpt{checkpoint_step}_daily.csv"
                ...
                for r in daily.itertuples(index=False):
                    ...
                    if i > 0 or n > 0:
                        action_events.append(f"DAP{int(round(float(r.dap)))} I{i:.0f}/N{n:.0f}")
                row = {
                    "station_code": STATION,
                    "site": SITE,
                    "year": int(year),
                    ...
                    "action_sequence": "; ".join(action_events),
...
def summarize_by_checkpoint(eval_summary: pd.DataFrame) -> pd.DataFrame:
    by = (
        ...
        .agg(
            ...
            unique_action_signatures=("action_sequence", "nunique"),
            max_swfac=("max_swfac", "max"),
            max_nstres=("max_nstres", "max"),
        )
        .sort_values(["checkpoint_step", "stage"])
    )
    return by


def write_record(out: Path, doc: Path, result: dict[str, Any], bc_log: pd.DataFrame, bc_meta: pd.DataFrame, train_inventory: pd.DataFrame, eval_summary: pd.DataFrame, by_ckpt: pd.DataFrame) -> None:
    lines = [
        ...
        "",
        md_table(
            bc_meta.groupby(["teacher_tier", "is_nonzero_action_day"], as_index=False)
            .agg(samples=("year", "count"), mean_weight=("sample_weight", "mean")),
            20,
        ),
        "",
        "## checkpoint 汇总",
        ...
        "## 训练模型清单",
        "",
        md_table(train_inventory, 20),
        "",
        "## 逐年验证结果",
        "",
        md_table(eval_summary, 120),
    ]
...
def dry_run(timesteps: int, checkpoint_steps: list[int], suffix: str) -> None:
    ...
    result = {
        ...
        "mode": "dry_run",
        "lowIC_input_root": base04302.LOWIC_INPUT_ROOT.relative_to(ROOT).as_posix(),
        "lowIC_input_root_exists": base04302.LOWIC_INPUT_ROOT.exists(),
        ...
        "teacher_action_mapping": "any positive irrigation -> 45 mm; any positive nitrogen -> 80 kg/ha",
        "observation_dim": len(base04302.ENHANCED_OBS_NAMES),
        "weather_features": base04302.WEATHER_FEATURE_NAMES,
        ...
        "bc_lr": BC_LR,
        "next_step_allowed": bool(base04302.LOWIC_INPUT_ROOT.exists() and PROMPT.exists() and not selected.empty),
    }
    print(json.dumps(result, indent=2, ensure_ascii=False))


def collect_bc_only(suffix: str) -> None:
    """Collect teacher-imitation samples and verify that mapped binary actions are legal."""
    out = out_for_suffix(suffix)
    ensure_dirs(out)
    selected = base04103.load_selected_teachers()
    config, env_config = build_config_and_env_config(YEARS)
    (out / "configs" / "043_05_env_config.json").write_text(json.dumps(env_config, indent=2, ensure_ascii=False), encoding="utf-8")
    (out / "configs" / "043_05_config.json").write_text(json.dumps(config, indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    action_counts = bc_meta["teacher_action_index"].value_counts().sort_index().to_dict()
    result = {
        "task": f"{TASK_ID}_{TASK_NAME}" + (f"_{suffix}" if suffix else ""),
        "mode": "collect_bc_only",
        "training_run": False,
        "dssat_episode_rollout": False,
        "teacher_mapping": "positive teacher irrigation/nitrogen mapped to binary I45/N80 timing action",
        "bc_sample_count": int(len(bc_meta)),
        "bc_year_count": int(bc_meta["year"].nunique()),
        "skipped_masked_teacher_action_count": int(len(skipped)),
        "action_index_counts": {str(k): int(v) for k, v in action_counts.items()},
        "observation_shape": list(dataset["obs"].shape),
        "mask_shape": list(dataset["masks"].shape),
        "outputs": {
            "bc_dataset_npz": (out / "bc_dataset" / "043_05_bc_dataset.npz").relative_to(ROOT).as_posix(),
            ...
            "config": (out / "configs" / "043_05_config.json").relative_to(ROOT).as_posix(),
            "env_config": (out / "configs" / "043_05_env_config.json").relative_to(ROOT).as_posix(),
            "result_json": (out / "043_05_collect_bc_only_result.json").relative_to(ROOT).as_posix(),
        },
    }
    (out / "043_05_collect_bc_only_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    print(json.dumps(result, indent=2, ensure_ascii=False))
...
def run_training(timesteps: int, checkpoint_steps: list[int], suffix: str) -> None:
    ...
    model = MaskablePPO(
        ...
        **base04302.base03222.base.ppo_kwargs(config),
    )
    bc_log = pretrain_policy_bc(model, dataset, out)
    bc_model = out / "models" / STATION / f"{STATION}_binary_forecast_teacher_warmstart_seed{SEED}_bc_init.zip"
    ...
    train_inventory_path = out / "evaluation" / "043_05_training_checkpoint_inventory.csv"
    train_inventory.to_csv(train_inventory_path, index=False, encoding="utf-8-sig")
    ...
    for row in train_inventory.itertuples(index=False):
        eval_df, _daily = evaluate_checkpoint(out, ROOT / row.model_path, int(row.checkpoint_step), str(row.stage), config, env_config, thresholds)
        eval_frames.append(eval_df)
    eval_summary = pd.concat(eval_frames, ignore_index=True)
    eval_summary_path = out / "evaluation" / "043_05_checkpoint_validation_summary.csv"
    eval_summary.to_csv(eval_summary_path, index=False, encoding="utf-8-sig")
    by_ckpt = summarize_by_checkpoint(eval_summary)
    by_ckpt_path = out / "evaluation" / "043_05_validation_summary_by_checkpoint.csv"
    by_ckpt.to_csv(by_ckpt_path, index=False, encoding="utf-8-sig")

    result = {
        "task": f"{TASK_ID}_{TASK_NAME}" + (f"_{suffix}" if suffix else ""),
        "branch": "A_binary_forecast_teacher_warmstart_training_completed",
        ...
        "outputs": {
            ...
            "bc_dataset_npz": (out / "bc_dataset" / "043_05_bc_dataset.npz").relative_to(ROOT).as_posix(),
            "bc_dataset_meta": (out / "bc_dataset" / "043_05_bc_dataset_meta.csv").relative_to(ROOT).as_posix(),
            "skipped_masked_teacher_actions": (out / "bc_dataset" / "043_05_bc_skipped_masked_teacher_actions.csv").relative_to(ROOT).as_posix(),
            "bc_log": (out / "logs" / "043_05_bc_pretrain_log.csv").relative_to(ROOT).as_posix(),
            "result_json": (out / "043_05_result.json").relative_to(ROOT).as_posix(),
        },
    }
    (out / "043_05_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    write_record(out, doc, result, bc_log, bc_meta, train_inventory, eval_summary, by_ckpt)
    print(json.dumps(result, indent=2, ensure_ascii=False))
...
def main() -> None:
    ...
    parser.add_argument("--collect-bc-only", action="store_true")
    ...
    elif args.collect_bc_only:
        collect_bc_only(args.suffix)
    else:
