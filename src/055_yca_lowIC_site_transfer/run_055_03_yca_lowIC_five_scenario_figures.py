"""055_03: YCA lowIC five-scenario daily and bar figures.

This is the YCA-local reporting entry point for the 055 site-transfer series.
It creates:
...
DEFAULT_CONFIG = ROOT / "configs" / "055_00_yca_lowIC_expanded_action_maskableppo.json"
DEFAULT_AUTO_CONFIG = ROOT / "configs" / "055_01_yca_lowIC_external_auto_n_rule_nstd050_minimal.json"
PROMPT = ROOT / "prompts" / "055_yca_lowIC_site_transfer_expanded_action_ppo_auto_static_level1_figures.md"
PPO_RUN_DIR = ROOT / "benchmark_results" / "055_00_yca_lowIC_expanded_action_maskableppo"
BASELINE_ROOT = ROOT / "benchmark_results" / "055_02_yca_lowIC_four_baselines_static_level1"
STATION = "YCA"
SITE = "YC"
...
def output_root(cfg: dict[str, Any], auto_config: dict[str, Any], checkpoint: int, label: str, run_id: str) -> Path:
    ...
    return ROOT / "benchmark_results" / (
        f"055_03_yca_{cfg['input_profile']}_{cfg['task_id']}_{cfg['task_name']}"
...
def replay_ppo_snapshot(cfg: dict[str, Any], checkpoint: int, year: int, out: Path, ppo_root: Path) -> Path:
    ...
    try:
        ...
        env = base03222.base.make_env(config, env_config, STATION, int(year), int(cfg.get("seed", 0)), f"YCA_{year}_055_03_replay", evaluation=True)
        obs, _info = env.reset()
        done, steps = False, 0
        ...
        if not np.isclose(replay_yield, saved_yield, rtol=0.0, atol=PPO_REPLAY_YIELD_ATOL_KG_HA):
            raise RuntimeError(
                f"PPO replay endpoint mismatch for YCA{year}: "
...
def plot_year(daily: pd.DataFrame, year: int, fig_dir: Path) -> Path:
    ...
    path = fig_dir / f"055_03_yca{year}_five_scenario_daily.png"
...
def main() -> None:
    ...
    if cfg.get("station_code") != STATION or auto_config.get("station_code") != STATION:
        raise ValueError("055_03 only accepts YCA PPO and YCA auto configs.")
    ...
    required = [
        ...
        auto_output_root / "evaluation" / "055_01_external_auto_n_summary.csv",
        baseline_root / "evaluation" / "055_02_baseline_summary.csv",
    ]
    for year in years:
        required.extend(
            [
    ...
    payload = {
        "task": "055_03_yca_five_scenario_figures",
    ...
    if args.dry_run:
        ...
        return
    if missing:
        raise FileNotFoundError("Missing required 055 figure inputs: " + "; ".join(missing))
    ...
    all_daily.to_csv(tab_dir / "055_03_yca_five_scenario_daily.csv", index=False, encoding="utf-8-sig")
    actions.to_csv(tab_dir / "055_03_yca_five_scenario_management_events.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(tab_dir / "055_03_yca_five_scenario_season_summary.csv", index=False, encoding="utf-8-sig")
    ...
    gaps_df.to_csv(tab_dir / "055_03_yca_metric_gaps_vs_four_baseline_max.csv", index=False, encoding="utf-8-sig")
    if not args.skip_bars:
        draw_grouped(summary, years, METRIC_COLUMNS, fig_dir / "055_03_yca_five_scenario_metrics.png")
        draw_grouped(summary, years, RESOURCE_COLUMNS, fig_dir / "055_03_yca_five_scenario_management.png")

    wins = gaps_df.groupby("metric")["ppo_minus_four_max"].apply(lambda values: int((pd.to_numeric(values, errors="coerce") > 0).sum())).to_dict()
    record = ROOT / "docs" / f"055_03_yca_{cfg['input_profile']}_{cfg['task_id']}_{cfg['task_name']}_five_scenario_figures_ckpt{checkpoint}_record.md"
    record.write_text(
        "\n".join(
            [
                f"# 055_03 YCA {cfg['input_profile']} five-scenario figures record",
                "",
                f"- PPO checkpoint: `{checkpoint}`.",
                f"- Years: `{years}`.",
                f"- PPO run dir: `{rel(ppo_root)}`.",
                f"- Baseline root: `{rel(baseline_root)}`.",
                "- Four non-PPO baselines are expected to come from 055_02 static level-1 corrected baseline outputs, not old 034_00 outputs.",
                ...
                f"- Daily table: `{rel(tab_dir / '055_03_yca_five_scenario_daily.csv')}`.",
                f"- Season summary: `{rel(tab_dir / '055_03_yca_five_scenario_season_summary.csv')}`.",
                f"- Gap table: `{rel(tab_dir / '055_03_yca_metric_gaps_vs_four_baseline_max.csv')}`.",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    payload.update(
        {
            ...
            "metric_bars": rel(fig_dir / "055_03_yca_five_scenario_metrics.png"),
            "management_bars": rel(fig_dir / "055_03_yca_five_scenario_management.png"),
            "daily_csv": rel(tab_dir / "055_03_yca_five_scenario_daily.csv"),
            "season_summary": rel(tab_dir / "055_03_yca_five_scenario_season_summary.csv"),
            "management_events": rel(tab_dir / "055_03_yca_five_scenario_management_events.csv"),
            "gaps": rel(tab_dir / "055_03_yca_metric_gaps_vs_four_baseline_max.csv"),
            "record_md": rel(record),
            "ppo_win_counts": wins,
        }
    )
    (out / "055_03_result.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2))
