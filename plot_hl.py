import sys
sys.path.insert(0, '/home/gymusr/workspace/gym_dssat_pdi_bingo/gym-dssat-pdi/local_packages')
import seaborn as sns  # 现在能找到了，且用的是虚拟环境里的numpy/matplotlib
import numpy as np
...
def parse_args():
    ...
    parser.add_argument('--mode', default='fertilization')
...
def plot_rewards(history_dict, mode, dos_cut=155, y_logscale=False, y_label=None, x_logscale=False, x_label=None, q_high=.95,
                 q_low=.05, saving_path=None, title=None, quantile_range_legend=True, diagnostics=False):
    reward_dict = get_rewards(history_dict)
    policy_names = [*reward_dict]
    # ========== 添加诊断代码 ==========
    if diagnostics:
        print("\n=== DIAGNOSTIC: plot_rewards ===")
        for policy_name in policy_names:
            rewards = reward_dict[policy_name]
            print(f"\nPolicy: {policy_name}")
            print(f"Number of episodes: {len(rewards)}")
            for i, reward in enumerate(rewards):
                print(f"  Episode {i}: type={type(reward)}, len={len(reward) if hasattr(reward, '__len__') else 'scalar'}")
                if hasattr(reward, '__len__'):
                    print(f"    First 5 values: {reward[:5]}")
...
def get_statistics(history_dict, mode):
    if mode == 'fertilization':
        features_subset = ['topwt', 'grnwt', 'cumsumfert', 'trnu']

        action_name = 'anfer'
    else:
        features_subset = ['topwt', 'grnwt', 'totir', 'runoff', 'trnu']
        action_name = 'amir'
    features = [*features_subset, 'trnu_mean', 'trnu_max', 'efficiency', 'duration', 'napp']

    state_features_dic = {key: {feature: [] for feature in features} for key in [*history_dict]}

    for key in [*history_dict]:
        for repetition_index, repetition in enumerate(history_dict[key]['observation']):
            last_state = repetition[-1]
            for feature in features_subset:
                values = last_state.get(feature, np.nan)
...
if __name__ == '__main__':
    ...
    mode = args.mode
    #mode = 'irrigation'
    input_dir = args.input_dir or f'./output_hl/{mode}'
    figure_dir = args.figure_dir or f'./figures_hl/{mode}'
    history_dict = load_data(path=f'{input_dir}/evaluation_histories.pkl')
    # 加在 load_data 后
    print("Sample PPO observation shape:", len(history_dict['ppo']['observation']), len(history_dict['ppo']['observation'][0]))
    print("Sample first step observation keys:", history_dict['ppo']['observation'][0][0].keys() if history_dict['ppo']['observation'] else "empty")
    ...
    print("Type of observation list:", type(history_dict['ppo']['observation'][0]))  # 应该输出 <class 'list'>
    print("Type of first observation dict:", type(history_dict['ppo']['observation'][0][0]))  # 应该输出 <class 'dict'>
    ...
    df_stages.transpose().round(0).to_csv(f'{input_dir}/growing_stages.csv', index_label='istage')

    action_keys = ['ppo', 'expert']
    plot_actions(history_dict=history_dict, mode=mode, saving_path=f'{figure_dir}/{mode}Applications.pdf',
                 keys=action_keys)
    plot_rewards(history_dict=history_dict, mode=mode, quantile_range_legend=False,
                 saving_path=f'{figure_dir}/{mode}Rewards.pdf')
    df_stats = get_statistics(history_dict=history_dict, mode=mode)
    df_stats.describe().round(1).to_csv(f'{input_dir}/advanced_evaluation.csv')
