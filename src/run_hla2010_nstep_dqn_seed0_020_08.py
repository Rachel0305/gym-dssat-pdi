...
OUT_ROOT = ROOT / "DSSAT_auto_validation" / "HLA_2004" / "hla2010_nstep_dqn_seed0_020_08"
...
def main() -> None:
    ...
    base.configure_shared_settings(); install_official_reward_module()
    null_baseline = hla.get_null_baseline_yield(2010)
    run_dir = OUT_ROOT / f"nstep5_seed0_{args.timesteps}steps"
    ...
    try:
        model = DQN(
            "MlpPolicy", train_env, verbose=0, seed=0,
        ...
        for checkpoint in range(args.checkpoint_interval, args.timesteps + 1, args.checkpoint_interval):
            ...
            previous = checkpoint
            model_dir = run_dir / "models"; model_dir.mkdir(parents=True, exist_ok=True)
        ...
        base.make_env = original_make_env; train_env.close()
    ...
    base.plot_checkpoints(daily, summary, run_dir / "figures" / f"hla2010_nstep5_seed0_{args.timesteps}steps.png", 2010, 0, args.timesteps)
    chosen = summary.sort_values(["total_reward", "checkpoint_step"], ascending=[False, True]).iloc[0]
    lines = [
        "# 020_08 HLA2010 n-step DQN seed0 记录", "",
        "- 唯一改动：标准DQN n_steps从1改为5。", "- Seed: 0。",
