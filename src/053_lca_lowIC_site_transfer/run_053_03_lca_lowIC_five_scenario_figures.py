"""053_03: LCA lowIC five-scenario daily and bar figures.

This is the LCA-local reporting entry point for the 053 site-transfer series.
It creates:
...
DEFAULT_AUTO_CONFIG = ROOT / "configs" / "053_01_lca_lowIC_external_auto_n_rule_nstd050_minimal.json"
PROMPT = ROOT / "prompts" / "053_lca_lowIC_site_transfer_expanded_action_ppo_auto_static_level1_figures.md"
PPO_RUN_DIR = ROOT / "benchmark_results" / "053_00_lca_lowIC_expanded_action_maskableppo"
BASELINE_ROOT = ROOT / "benchmark_results" / "053_02_lca_lowIC_four_baselines_static_level1"
STATION = "LCA"
SITE = "LC"
...
def output_root(cfg: dict[str, Any], auto_config: dict[str, Any], checkpoint: int, label: str, run_id: str) -> Path:
    ...
    return ROOT / "benchmark_results" / (
        f"053_03_lca_{cfg['input_profile']}_{cfg['task_id']}_{cfg['task_name']}"
...
def replay_ppo_snapshot(cfg: dict[str, Any], checkpoint: int, year: int, out: Path, ppo_root: Path) -> Path:
    ...
    if len(expected) != 1:
        raise RuntimeError(f"Expected one PPO validation row for LCA{year} checkpoint {checkpoint}")
    ...
    if target.exists() and (target / "Summary.OUT").exists():
        ...
        # DSSAT daily OUT tables may be rounded more coarsely than the saved
        # validation summary. Treat sub-1 kg/ha endpoint differences as the
        # same replay endpoint, while still catching real trajectory mismatch.
        if np.isclose(replay_yield, saved_yield, rtol=0.0, atol=PPO_REPLAY_YIELD_ATOL_KG_HA):
            return target
    ...
    try:
        ...
        env = base03222.base.make_env(config, env_config, STATION, int(year), int(cfg.get("seed", 0)), f"LCA_{year}_053_03_replay", evaluation=True)
        obs, _info = env.reset()
        done, steps = False, 0
        while not done and steps < 260:
            ...
            steps += 1
        if not done:
            raise RuntimeError(f"PPO replay did not finish within 260 steps for LCA{year}")
        ...
        if not np.isclose(replay_yield, saved_yield, rtol=0.0, atol=PPO_REPLAY_YIELD_ATOL_KG_HA):
            raise RuntimeError(
                f"PPO replay endpoint mismatch for LCA{year}: "
...
def plot_year(daily: pd.DataFrame, year: int, fig_dir: Path) -> Path:
    ...
    fig.suptitle(f"LCA{year} five-scenario daily process", x=0.02, ha="left", fontweight="bold")
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    path = fig_dir / f"053_03_lca{year}_five_scenario_daily.png"
...
def main() -> None:
    ...
    if cfg.get("station_code") != STATION or auto_config.get("station_code") != STATION:
        raise ValueError("053_03 only accepts LCA PPO and LCA auto configs.")
    ...
    required = [
        ...
        auto_output_root / "evaluation" / "053_01_external_auto_n_summary.csv",
        baseline_root / "evaluation" / "053_02_baseline_summary.csv",
    ]
    for year in years:
        required.extend(
            [
    ...
    payload = {
        "task": "053_03_lca_five_scenario_figures",
    ...
    if args.dry_run:
        ...
        return
    if missing:
        raise FileNotFoundError("Missing required 053 figure inputs: " + "; ".join(missing))
    ...
    all_daily.to_csv(tab_dir / "053_03_lca_five_scenario_daily.csv", index=False, encoding="utf-8-sig")
    actions.to_csv(tab_dir / "053_03_lca_five_scenario_management_events.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(tab_dir / "053_03_lca_five_scenario_season_summary.csv", index=False, encoding="utf-8-sig")
    ...
    gaps_df.to_csv(tab_dir / "053_03_lca_metric_gaps_vs_four_baseline_max.csv", index=False, encoding="utf-8-sig")
    if not args.skip_bars:
        draw_grouped(summary, years, METRIC_COLUMNS, fig_dir / "053_03_lca_five_scenario_metrics.png")
        draw_grouped(summary, years, RESOURCE_COLUMNS, fig_dir / "053_03_lca_five_scenario_management.png")

    wins = gaps_df.groupby("metric")["ppo_minus_four_max"].apply(lambda values: int((pd.to_numeric(values, errors="coerce") > 0).sum())).to_dict()
    record = ROOT / "docs" / f"053_03_lca_{cfg['input_profile']}_{cfg['task_id']}_{cfg['task_name']}_five_scenario_figures_ckpt{checkpoint}_record.md"
    record.write_text(
        "\n".join(
            [
                f"# 053_03 LCA {cfg['input_profile']} five-scenario figures record",
                "",
                f"- PPO checkpoint: `{checkpoint}`.",
                f"- Years: `{years}`.",
                f"- PPO run dir: `{rel(ppo_root)}`.",
                f"- Baseline root: `{rel(baseline_root)}`.",
                "- Four non-PPO baselines are expected to come from 053_02 static level-1 corrected baseline outputs, not old 034_00 outputs.",
                ...
                f"- Daily table: `{rel(tab_dir / '053_03_lca_five_scenario_daily.csv')}`.",
                f"- Season summary: `{rel(tab_dir / '053_03_lca_five_scenario_season_summary.csv')}`.",
                f"- Gap table: `{rel(tab_dir / '053_03_lca_metric_gaps_vs_four_baseline_max.csv')}`.",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    payload.update(
        {
            ...
            "metric_bars": rel(fig_dir / "053_03_lca_five_scenario_metrics.png"),
            "management_bars": rel(fig_dir / "053_03_lca_five_scenario_management.png"),
            "daily_csv": rel(tab_dir / "053_03_lca_five_scenario_daily.csv"),
            "season_summary": rel(tab_dir / "053_03_lca_five_scenario_season_summary.csv"),
            "management_events": rel(tab_dir / "053_03_lca_five_scenario_management_events.csv"),
            "gaps": rel(tab_dir / "053_03_lca_metric_gaps_vs_four_baseline_max.csv"),
            "record_md": rel(record),
            "ppo_win_counts": wins,
        }
    )
    (out / "053_03_result.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2))
