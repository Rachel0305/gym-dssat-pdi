"""049_02: HLA originIC five-scenario daily and bar figures.

This is the HLA-local reporting entry point for the 049 site-transfer series.
It creates:
...
DEFAULT_CONFIG = ROOT / "configs" / "049_00_hla_originIC_expanded_action_maskableppo.json"
DEFAULT_AUTO_CONFIG = ROOT / "configs" / "049_01_hla_originIC_external_auto_n_rule_nstd050_minimal.json"
PROMPT = ROOT / "prompts" / "049_hla_originIC_site_transfer_expanded_action_ppo_and_auto.md"
PPO_RUN_DIR = ROOT / "benchmark_results" / "049_00_hla_originIC_expanded_action_maskableppo"
BASELINE_ROOT = ROOT / "benchmark_results" / "034_00_multisite_input_ic1_four_baseline_rebuild"
STATION = "HLA"
SITE = "HLA"
...
def output_root(cfg: dict[str, Any], auto_config: dict[str, Any], checkpoint: int, label: str, run_id: str) -> Path:
    ...
    return ROOT / "benchmark_results" / (
        f"049_02_hla_{cfg['input_profile']}_{cfg['task_id']}_{cfg['task_name']}"
...
def replay_ppo_snapshot(cfg: dict[str, Any], checkpoint: int, year: int, out: Path, ppo_root: Path) -> Path:
    ...
    try:
        ...
        env = base03222.base.make_env(config, env_config, STATION, int(year), int(cfg.get("seed", 0)), f"HLA_{year}_049_02_replay", evaluation=True)
        obs, _info = env.reset()
        done, steps = False, 0
        ...
        if not np.isclose(replay_yield, saved_yield, rtol=0.0, atol=0.5):
            raise RuntimeError(f"PPO replay endpoint mismatch for HLA{year}: saved={saved_yield:.4f}, replay={replay_yield:.4f}")
        return target
    finally:
        if env is not None:
            env.close()
...
def plot_year(daily: pd.DataFrame, year: int, fig_dir: Path) -> Path:
    ...
    path = fig_dir / f"049_02_hla{year}_five_scenario_daily.png"
...
def main() -> None:
    ...
    if cfg.get("station_code") != STATION or auto_config.get("station_code") != STATION:
        raise ValueError("049_02 only accepts HLA PPO and HLA auto configs.")
    ...
    required = [
        ...
        auto_output_root / "evaluation" / "049_01_external_auto_n_summary.csv",
        baseline_root / "evaluation" / "034_00_full_baseline_summary.csv",
    ]
    for year in years:
        required.extend(
            [
    ...
    payload = {
        "task": "049_02_hla_five_scenario_figures",
    ...
    if args.dry_run:
        ...
        return
    if missing:
        raise FileNotFoundError("Missing required 049 figure inputs: " + "; ".join(missing))
    ...
    all_daily.to_csv(tab_dir / "049_02_hla_five_scenario_daily.csv", index=False, encoding="utf-8-sig")
    actions.to_csv(tab_dir / "049_02_hla_five_scenario_management_events.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(tab_dir / "049_02_hla_five_scenario_season_summary.csv", index=False, encoding="utf-8-sig")
    ...
    gaps_df.to_csv(tab_dir / "049_02_hla_metric_gaps_vs_four_baseline_max.csv", index=False, encoding="utf-8-sig")
    if not args.skip_bars:
        draw_grouped(summary, years, METRIC_COLUMNS, fig_dir / "049_02_hla_five_scenario_metrics.png")
        draw_grouped(summary, years, RESOURCE_COLUMNS, fig_dir / "049_02_hla_five_scenario_management.png")

    wins = gaps_df.groupby("metric")["ppo_minus_four_max"].apply(lambda values: int((pd.to_numeric(values, errors="coerce") > 0).sum())).to_dict()
    record = ROOT / "docs" / f"049_02_hla_{cfg['input_profile']}_{cfg['task_id']}_{cfg['task_name']}_five_scenario_figures_ckpt{checkpoint}_record.md"
    record.write_text(
        "\n".join(
            [
                f"# 049_02 HLA {cfg['input_profile']} five-scenario figures record",
                "",
                ...
                f"- Daily table: `{rel(tab_dir / '049_02_hla_five_scenario_daily.csv')}`.",
                f"- Season summary: `{rel(tab_dir / '049_02_hla_five_scenario_season_summary.csv')}`.",
                f"- Gap table: `{rel(tab_dir / '049_02_hla_metric_gaps_vs_four_baseline_max.csv')}`.",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    payload.update(
        {
            ...
            "metric_bars": rel(fig_dir / "049_02_hla_five_scenario_metrics.png"),
            "management_bars": rel(fig_dir / "049_02_hla_five_scenario_management.png"),
            "daily_csv": rel(tab_dir / "049_02_hla_five_scenario_daily.csv"),
            "season_summary": rel(tab_dir / "049_02_hla_five_scenario_season_summary.csv"),
            "management_events": rel(tab_dir / "049_02_hla_five_scenario_management_events.csv"),
            "gaps": rel(tab_dir / "049_02_hla_metric_gaps_vs_four_baseline_max.csv"),
            "record_md": rel(record),
            "ppo_win_counts": wins,
        }
    )
    (out / "049_02_result.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2))
