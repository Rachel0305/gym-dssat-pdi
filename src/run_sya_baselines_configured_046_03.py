"""046_03: config-driven SYA originIC four-baseline rebuild.
...
PROMPT = ROOT / "prompts" / "046_03_sya_originIC_four_baselines.md"
INPUT_PROFILES = {
...
def read_config(path: Path) -> dict[str, Any]:
    ...
    if str(cfg.get("station_code")) != "SYA":
        raise ValueError("046_03 仅适用于 SYA")
    ...
    if not cfg.get("scope", {}).get("validation_years"):
        raise ValueError("配置缺少 validation_years")
    return cfg
...
def output_root(cfg: dict[str, Any], run_id: str = "") -> Path:
    return ROOT / "benchmark_results" / f"046_03_sya_{cfg['input_profile']}_four_baselines{run_suffix(run_id)}"


def preflight(cfg: dict[str, Any]) -> dict[str, Any]:
    input_root = INPUT_PROFILES[str(cfg["input_profile"])]
    mzx = input_root / "SY" / "CNSY1201.MZX"
    schedule = recorded_template_schedules_dedup().get("SY", {})
    issues = []
    if not input_root.exists():
        issues.append("input root missing")
    if not mzx.exists():
        issues.append("CNSY1201.MZX missing")
    return {
        "station_code": "SYA",
...
def recorded_template_schedules_dedup() -> dict[str, dict[int, dict[str, float]]]:
    ...
    The old 027_05 daily table stores the same recorded SY2014 line under both
...
def selected_rows(years: list[int]) -> pd.DataFrame:
    ...
    if sorted(out["year"].tolist()) != sorted(years):
        raise RuntimeError("配置验证年份不完整地存在于 split 表中")
...
def write_record(out: Path, cfg: dict[str, Any], pf: dict[str, Any], summary: pd.DataFrame, manifest: pd.DataFrame, elapsed: float) -> Path:
    doc = ROOT / "docs" / f"046_03_sya_{cfg['input_profile']}_four_baselines_record.md"
    ...
    lines = [
        f"# 046_03 SYA {cfg['input_profile']} 四情景基线记录",
        "",
        f"- 输入目录：`{pf['resolved_input_root']}`",
        f"- SY 模板 SHA256：`{pf['source_mzx_sha256']}`",
        f"- 运行耗时：{elapsed:.1f} s",
        "- recorded farmer template 是静态站点模板复用，不代表每年的真实田间记录。",
        f"- recorded 模板总灌溉/总施氮：{pf['recorded_template_total_irrigation_mm']} mm / {pf['recorded_template_total_nitrogen_kg_ha']} kg/ha。",
        "",
        "## 情景均值",
        "",
    ]
    for frame in [by_scenario, manifest[manifest["status"].ne("ok")].copy() if not manifest.empty else pd.DataFrame()]:
        if frame.empty:
            lines.append("_无记录_\n")
            continue
...
def run(cfg_path: Path, dry_run: bool, years_override: list[int] | None = None, run_id: str = "") -> dict[str, Any]:
    ...
    if dry_run:
        return {"task": "046_03_sya_configured_four_baselines", "mode": "dry_run", **pf, "output_root": rel(out)}
    ...
    selected.to_csv(out / "configs" / "046_03_selected_years.csv", index=False, encoding="utf-8-sig")
    run_config, env_config = build_configs(out, selected)
    direct_ppo.write_yaml(env_config, out / "configs" / "046_03_resolved_env_config.yaml")
    ...
    try:
        for row in selected.itertuples(index=False):
            for scenario in SCENARIOS:
                print(f"[046_03] SYA{int(row.year)} {scenario}", flush=True)
                try:
                    daily, summary, checks = baseline.evaluate_scenario(run_config, env_config, "SYA", int(row.year), scenario, schedules)
                    summary["source_input_profile"] = cfg["input_profile"]
                    summary["source_status"] = "generated_046_03_configured_baseline"
                    summary["render_check_count"] = len(checks)
                    daily["source_input_profile"] = cfg["input_profile"]
                    daily_frames.append(daily)
                    summary_rows.append(summary)
                    schedule = {} if scenario == "dssat_auto" else baseline.schedule_for(scenario, "SY", schedules)
    ...
    summary.to_csv(out / "evaluation" / "046_03_baseline_summary.csv", index=False, encoding="utf-8-sig")
    daily.to_csv(out / "evaluation" / "046_03_baseline_daily.csv", index=False, encoding="utf-8-sig")
    manifest.to_csv(out / "evaluation" / "046_03_coverage_manifest.csv", index=False, encoding="utf-8-sig")
    (out / "046_03_preflight.json").write_text(json.dumps(pf, indent=2, ensure_ascii=False), encoding="utf-8")
    record = write_record(out, cfg, pf, summary, manifest, time.time() - started)
    result = {"task": "046_03_sya_configured_four_baselines", "output_root": rel(out), "record_md": rel(record), "successful_runs": int(len(summary)), "non_ok_runs": int((manifest["status"] != "ok").sum()) if not manifest.empty else 0}
    (out / "046_03_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    return result
