...
完成五站点 half-split MaskablePPO 重跑；033_05 已确认该输入链在可用站点年份中满足：

- 源模板、天气、土壤、品种均来自 `multisite_new_cultivar_inputs_013`；
- 渲染后 treatment 1 启用 `IC=1, MI=1, MF=1`；
- WSTA 与目标年份 WTH 文件一致；
- 无 fallback 到旧 `my_data`。

旧四情景基线来自旧输入链或历史拼接结果，不能直接与 033_04 PPO 结果比较。因此本任务先在统一输入链下重建四情景基线，作为后续正式比较、作图和汇报的统一尺子。
...
- `recorded_farmer_template` 与 `official_extension_expert` 的固定灌溉/施氮措施必须写入渲染后的 `.MZX` 静态管理表（`@I`/`@F` 行），step 阶段只发送 no-op；
- 不得用外部 RL action 累计值冒充 DSSAT 实际执行措施；
