...
OUT_DIR = PROJECT_ROOT / "DSSAT_auto_validation" / "yc2014_unified_dqn_checkpoint_diagnostic_015_04"
DOC_PATH = PROJECT_ROOT / "docs" / "2026-06-30_015_04_yc2014_unified_dqn_checkpoint_diagnostic_record.md"
...
WATER_COST = 1.0
NITROGEN_COST = 5.0
SCENARIO = "dqn_unified_checkpoint"
...
def configure_yc_module() -> None:
    ...
    yc.SCENARIO_LABELS = {SCENARIO: "Unified DQN checkpoint"}
...
def evaluate_model(model, env_args: dict, checkpoint_step: int) -> tuple[pd.DataFrame, dict]:
    ...
    events = yc.parse_events_eval(OUT_DIR / f"seed{SEED}" / f"checkpoint_{checkpoint_step}", f"checkpoint_{checkpoint_step}")
    plantgro = parse_dssat_table(OUT_DIR / f"seed{SEED}" / f"checkpoint_{checkpoint_step}" / "pdi_tmp_snapshot_eval" / "PlantGro.OUT")
    summary = {
        "checkpoint_step": checkpoint_step,
...
def plot_checkpoints(all_daily: pd.DataFrame, summary: pd.DataFrame, out_path: Path) -> None:
    ...
    axes[0].set_title("YC2014 unified DQN checkpoint diagnostic, 50K training", loc="left", fontsize=13)
...
def main() -> None:
    ...
    try:
        ...
        while completed < TOTAL_TIMESTEPS:
            ...
            print(f"[015_04] learn chunk {completed} -> {completed + target}", flush=True)
            model.learn(total_timesteps=target, reset_num_timesteps=(completed == 0), progress_bar=False)
            completed += target
            print(f"[015_04] evaluate checkpoint {completed}", flush=True)
            daily, summary = evaluate_model(model, env_args, completed)
            daily_path = run_out_dir / f"015_04_yc2014_checkpoint_{completed // 1000}k_daily.csv"
    ...
    daily_path = run_out_dir / "015_04_yc2014_unified_dqn_checkpoint_daily.csv"
    summary_path = run_out_dir / "015_04_yc2014_unified_dqn_checkpoint_summary.csv"
    fig_path = fig_dir / "yc2014_unified_dqn_checkpoint_diagnostic.png"
    ...
    print(summary_df[["checkpoint_step", "action_irrigation_total", "action_fertilizer_total", "final_grain_kg_ha", "final_biomass_kg_ha", "max_water_stress", "max_nitrogen_stress", "total_reward"]].to_string(index=False))
