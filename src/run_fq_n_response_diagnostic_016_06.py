...
from run_deterministic_oracle_upper_bound_scan_016_05 import (
    prepare_fq_text,
    parse_dssat_table,
)

OUT_DIR = PROJECT_ROOT / "DSSAT_auto_validation" / "fq_n_response_diagnostic_016_06"
DOC_PATH = PROJECT_ROOT / "docs" / "2026-07-05_016_06_fq_n_response_diagnostic_record.md"
YEARS = [2008, 2016]
N_LEVELS = [0, 25, 50, 75, 100, 150, 200, 250, 300]
FIXED_IRR_TOTAL = 60.0
IRR_DATES = [45, 70]
N_DATES = [1, 40, 65]


def split_n(total: float) -> dict[int, float]:
    if total <= 0:
        return {}
    ...
    for dap in N_DATES:
        if remaining <= 1e-9:
            break
        amt = min(100.0, remaining)
        out[dap] = amt
        remaining -= amt
    if remaining > 1e-6:
        out[N_DATES[-1]] = out.get(N_DATES[-1], 0.0) + remaining
    return out


def fixed_schedule(total_n: float) -> dict[int, dict[str, float]]:
    irr = {45: 30.0, 70: 30.0}
    nit = split_n(total_n)
    daps = sorted(set(irr) | set(nit))
    return {dap: {"amir": irr.get(dap, 0.0), "anfer": nit.get(dap, 0.0)} for dap in daps}


def prepare_run_dir(year: int, total_n: int) -> Path:
    scenario = f"I60_critical_N{total_n}"
...
def run_one(year: int, total_n: int, timeout_s: int) -> dict[str, Any]:
    run_dir = prepare_run_dir(year, total_n)
    schedule = fixed_schedule(total_n)
...
def write_record(df: pd.DataFrame) -> None:
    ...
    for year, g in df.groupby("year"):
        ...
        n0 = g.loc[g["n_total"].eq(0), "final_gwad"].iloc[0]
        n150 = g.loc[g["n_total"].eq(150), "final_gwad"].iloc[0]
        n300 = g.loc[g["n_total"].eq(300), "final_gwad"].iloc[0]
        lines.append(
            f"- FQ{year}: 在固定 `I60 critical` 下，`N0={n0:.1f}`, `N150={n150:.1f}`, `N300={n300:.1f}`, `best={best:.1f}`。"
        )
    DOC_PATH.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    rows = []
    for year in YEARS:
        for n_total in N_LEVELS:
            print(f"[016_06] FQ{year} N={n_total}", flush=True)
            rows.append(run_one(year, n_total, args.timeout_s))
    df = pd.DataFrame(rows)
    out_csv = OUT_DIR / "016_06_fq_n_response_summary.csv"
