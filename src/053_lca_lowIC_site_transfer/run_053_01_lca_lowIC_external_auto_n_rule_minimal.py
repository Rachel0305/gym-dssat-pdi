"""053_01: LCA lowIC minimal external auto-N baseline.
...
DEFAULT_CONFIG = ROOT / "configs" / "053_01_lca_lowIC_external_auto_n_rule_nstd050_minimal.json"
...
def read_config(path: Path) -> dict[str, Any]:
    ...
    if cfg.get("station_code") != STATION or cfg.get("site") != SITE:
        raise ValueError("053_01 is registered for station_code=LCA and site=LC.")
    ...
    if present:
        raise ValueError(f"053_01 minimal auto-N must not include removed constraints: {present}")
    return cfg
...
def evaluate_year(run_config: dict[str, Any], env_config: dict[str, Any], year: int, rule: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any], dict[str, Any]]:
    env, env_args, rendered_text = make_env_auto_irrigation_external_n(env_config, year, f"LCA_{year}_053_01_external_auto_n")
...
def write_record(out: Path, cfg: dict[str, Any], summary: pd.DataFrame, failures: pd.DataFrame, elapsed: float) -> Path:
    ...
    lines = [
        f"# {cfg['task_id']} LCA lowIC minimal external auto-N record",
        "",
        ...
        f"- Summary CSV: `{rel(out / 'evaluation' / '053_01_external_auto_n_summary.csv')}`.",
    ]
...
def run(cfg_path: Path, dry_run: bool, years_override: list[int] | None = None, run_id: str = "") -> dict[str, Any]:
    ...
    pf = {
        "task": "053_01_lca_external_auto_n_rule_minimal",
    ...
    direct_ppo.write_yaml(env_config, out / "configs" / "053_01_resolved_env_config.yaml")
    ...
    try:
        for year in years:
            print(f"[053_01] LCA{year} {SCENARIO}", flush=True)
            try:
    ...
    summary.to_csv(out / "evaluation" / "053_01_external_auto_n_summary.csv", index=False, encoding="utf-8-sig")
    daily.to_csv(out / "evaluation" / "053_01_external_auto_n_daily.csv", index=False, encoding="utf-8-sig")
    rendered_df.to_csv(out / "evaluation" / "053_01_rendered_management_mode_audit.csv", index=False, encoding="utf-8-sig")
    failures.to_csv(out / "evaluation" / "053_01_failures.csv", index=False, encoding="utf-8-sig")
    ...
    (out / "053_01_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    return result
