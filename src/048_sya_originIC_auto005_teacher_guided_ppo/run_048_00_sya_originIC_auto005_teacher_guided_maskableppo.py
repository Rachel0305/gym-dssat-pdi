"""048_00: SYA originIC auto-0.05 teacher-guided MaskablePPO experiment.
...
DEFAULT_CONFIG = TASK_DIR / "config_048_00_sya_originIC_auto005_teacher_guided_maskableppo.json"
PROMPT = ROOT / "prompts" / "048_00_sya_originIC_auto005_teacher_guided_maskableppo.md"
...
def read_config(path: Path) -> dict[str, Any]:
    ...
    if irrigation != EXPECTED_IRRIGATION_LEVELS or nitrogen != EXPECTED_NITROGEN_LEVELS:
        raise ValueError(
            "048_00 action contract requires irrigation [0,15,30,45] and nitrogen [0,40,80,120]."
        )
    obs = cfg.get("observation_contract", {})
    if obs.get("base") != "046_02_raw_observation":
        raise ValueError("048_00 must inherit the 046_02 raw observation.")
    if bool(obs.get("normalization_enabled", True)) or bool(obs.get("weather_forecast_enabled", True)):
        raise ValueError("048_00 forbids observation normalization and weather forecast features.")
    if cfg.get("reference_run") != "046_10_sya_originIC_expanded_action_maskableppo":
        raise ValueError("048_00 must reference the 046_10 expanded-action PPO baseline.")
    teacher = cfg.get("teacher_guidance", {})
    if not bool(teacher.get("enabled", False)):
        raise ValueError("048_00 requires teacher guidance to be enabled.")
    if float(teacher.get("nitrogen_stress_threshold", -1.0)) != 0.05:
        raise ValueError("048_00 teacher must use nitrogen_stress_threshold=0.05.")
    return cfg
