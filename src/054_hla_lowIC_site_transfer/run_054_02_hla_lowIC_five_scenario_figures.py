"""054_02: HLA lowIC five-scenario daily and bar figures.
...
def output_root(cfg: dict[str, Any], auto_config: dict[str, Any], checkpoint: int, label: str, run_id: str) -> Path:
    ...
    return ROOT / "benchmark_results" / (
        f"054_02_hla_{cfg['input_profile']}_{cfg['task_id']}_{cfg['task_name']}"
...
def replay_ppo_snapshot(cfg: dict[str, Any], checkpoint: int, year: int, out: Path, ppo_root: Path) -> Path:
    ...
    try:
        ...
        env = base03222.base.make_env(config, env_config, STATION, int(year), int(cfg.get("seed", 0)), f"HLA_{year}_054_02_replay", evaluation=True)
        obs, _info = env.reset()
        done, steps = False, 0
...
def plot_year(daily: pd.DataFrame, year: int, fig_dir: Path) -> Path:
    ...
    path = fig_dir / f"054_02_hla{year}_five_scenario_daily.png"
...
def main() -> None:
    ...
    if cfg.get("station_code") != STATION or auto_config.get("station_code") != STATION:
        raise ValueError("054_02 only accepts HLA PPO and HLA auto configs.")
    ...
    payload = {
        "task": "054_02_hla_five_scenario_figures",
    ...
    all_daily.to_csv(tab_dir / "054_02_hla_five_scenario_daily.csv", index=False, encoding="utf-8-sig")
    actions.to_csv(tab_dir / "054_02_hla_five_scenario_management_events.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(tab_dir / "054_02_hla_five_scenario_season_summary.csv", index=False, encoding="utf-8-sig")
    ...
    gaps_df.to_csv(tab_dir / "054_02_hla_metric_gaps_vs_four_baseline_max.csv", index=False, encoding="utf-8-sig")
    if not args.skip_bars:
        draw_grouped(summary, years, METRIC_COLUMNS, fig_dir / "054_02_hla_five_scenario_metrics.png")
        draw_grouped(summary, years, RESOURCE_COLUMNS, fig_dir / "054_02_hla_five_scenario_management.png")

    wins = gaps_df.groupby("metric")["ppo_minus_four_max"].apply(lambda values: int((pd.to_numeric(values, errors="coerce") > 0).sum())).to_dict()
    record = ROOT / "docs" / f"054_02_hla_{cfg['input_profile']}_{cfg['task_id']}_{cfg['task_name']}_five_scenario_figures_ckpt{checkpoint}_record.md"
    record.write_text(
        "\n".join(
            [
                f"# 054_02 HLA {cfg['input_profile']} five-scenario figures record",
                "",
                ...
                f"- Daily table: `{rel(tab_dir / '054_02_hla_five_scenario_daily.csv')}`.",
                f"- Season summary: `{rel(tab_dir / '054_02_hla_five_scenario_season_summary.csv')}`.",
                f"- Gap table: `{rel(tab_dir / '054_02_hla_metric_gaps_vs_four_baseline_max.csv')}`.",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    payload.update(
        {
            ...
            "metric_bars": rel(fig_dir / "054_02_hla_five_scenario_metrics.png"),
            "management_bars": rel(fig_dir / "054_02_hla_five_scenario_management.png"),
            "daily_csv": rel(tab_dir / "054_02_hla_five_scenario_daily.csv"),
            "season_summary": rel(tab_dir / "054_02_hla_five_scenario_season_summary.csv"),
            "management_events": rel(tab_dir / "054_02_hla_five_scenario_management_events.csv"),
            "gaps": rel(tab_dir / "054_02_hla_metric_gaps_vs_four_baseline_max.csv"),
            "record_md": rel(record),
            "ppo_win_counts": wins,
        }
    )
    (out / "054_02_result.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2))
