...
OUT_DIR = PROJECT_ROOT / "DSSAT_auto_validation" / "multisite_new_cultivar_yc2014_action_window_comparison_013_04"
DOC_PATH = PROJECT_ROOT / "docs" / "2026-06-30_013_04_yc2014_action_window_comparison_record.md"
...
SCENARIO_ORDER = ["null", "recorded", "dssat_auto", "dqn_free_daily", "dqn_agronomic_window"]
SCENARIO_LABELS = {
    "null": "Null",
    "recorded": "Recorded",
    "dssat_auto": "DSSAT auto",
    "dqn_free_daily": "DQN free daily",
    "dqn_agronomic_window": "DQN agronomic window",
}
SCENARIO_COLORS = {
    "null": "#464C55",
    "recorded": "#CC6F47",
    "dssat_auto": "#5477C4",
    "dqn_free_daily": "#386411",
    "dqn_agronomic_window": "#7A2E8E",
}
...
def prepare_case_for_scenario(scenario: str) -> Path:
    ...
    text = prepare_text_for_scenario(source, TRNO, scenario if not scenario.startswith("dqn_") else "null")
...
def main() -> None:
    ...
    parser.add_argument("--only", choices=["null", "recorded", "dssat_auto", "free", "window"], default=None)
    ...
    if args.only in (None, "free"):
        dqn_daily, dqn_summary = run_dqn_smoke("dqn_free_daily", FREE_DAILY_WINDOWS)
        all_daily.append(dqn_daily)
        summaries.append(dqn_summary)
    if args.only in (None, "window"):
        dqn_daily, dqn_summary = run_dqn_smoke("dqn_agronomic_window", AGRONOMIC_WINDOWS)
        all_daily.append(dqn_daily)
        summaries.append(dqn_summary)
    ...
    all_daily_df.to_csv(OUT_DIR / "013_04_yc2014_action_window_comparison_daily.csv", index=False, encoding="utf-8-sig")
    summary_df.to_csv(OUT_DIR / "013_04_yc2014_action_window_comparison_summary.csv", index=False, encoding="utf-8-sig")
    process_plot(all_daily_df, figures_dir / "yc2014_action_window_comparison_process.png")
