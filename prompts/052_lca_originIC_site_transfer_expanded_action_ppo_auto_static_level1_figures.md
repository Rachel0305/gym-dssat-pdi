# 052 LCA/LC originIC site-transfer experiment prompt
...
Run LC under the same controlled workflow:

1. `052_02`: corrected four-baseline rebuild.
2. `052_00`: frozen 046_10-style expanded-action MaskablePPO.
3. `052_01`: DSSAT native automatic irrigation plus minimal external auto-N.
4. `052_03`: yearly five-scenario daily plots and season-level bar charts.
...
`configs/sites/lca.yaml` notes that a historical single-year adapter exists for LC2010, but the current 052 station-screening series intentionally keeps the same multisite originIC pipeline used by 050/051. Do not silently switch to the 017_11 adapter unless a separate LC-specific experiment is opened.
