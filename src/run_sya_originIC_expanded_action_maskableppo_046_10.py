"""046_10: isolated originIC 16-action MaskablePPO experiment.

This runner preserves the completed 046_02 decision environment and changes
only the discrete dose grid.  Its smoke audit is grid-aware: it verifies that
actions are legal, transmitted into the environment, diverse, and not all
concentrated at DAP 1 before a 100K run is considered.
"""
...
DEFAULT_CONFIG = ROOT / "configs" / "046_10_sya_originIC_expanded_action_maskableppo.json"
PROMPT = ROOT / "prompts" / "046_10_sya_originIC_expanded_action_maskableppo.md"
...
def read_config(path: Path) -> dict[str, Any]:
    ...
    if irrigation != EXPECTED_IRRIGATION_LEVELS or nitrogen != EXPECTED_NITROGEN_LEVELS:
        raise ValueError(
            "046_10 action contract requires irrigation [0,15,30,45] and nitrogen [0,40,80,120]."
        )
    obs = cfg.get("observation_contract", {})
    if obs.get("base") != "046_02_raw_observation":
        raise ValueError("046_10 must inherit the 046_02 raw observation.")
    if bool(obs.get("normalization_enabled", True)) or bool(obs.get("weather_forecast_enabled", True)):
        raise ValueError("046_10 forbids observation normalization and weather forecast features.")
    if cfg.get("reference_run") != "046_02_sya_originIC_binary_timing_ppo":
        raise ValueError("046_10 must reference the frozen 046_02 baseline.")
    return cfg
