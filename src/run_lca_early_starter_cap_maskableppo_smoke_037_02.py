...
TASK = "037_02_lca_early_starter_cap_maskableppo_smoke"
...
def write_record(
    ...
    lines: list[str] = []
    lines.append("# 037_02：LCA early starter cap 自由时序 MaskablePPO smoke 记录\n")
    lines.append("\n## 任务边界\n")
...
def main() -> None:
    ...
    direct_ppo.write_yaml(config, OUT / "configs" / "037_02_config_snapshot.yaml")
    direct_ppo.write_yaml(env_config, OUT / "configs" / "037_02_resolved_env_config.yaml")
    split.to_csv(OUT / "configs" / "037_02_lca_half_split_years.csv", index=False, encoding="utf-8-sig")

    rows: list[dict[str, Any]] = []
    reset_df = pd.DataFrame()
    try:
        train_df, reset_df = batch.train_station(config, env_config, STATION, train_years)
        train_df.to_csv(OUT / "evaluation" / "037_02_training_checkpoint_inventory.csv", index=False, encoding="utf-8-sig")
        reset_df.to_csv(OUT / "logs" / "037_02_training_year_reset_counts.csv", index=False, encoding="utf-8-sig")
        ...
        (OUT / "037_02_error.txt").write_text(err, encoding="utf-8")
        raise
    ...
    eval_df.to_csv(OUT / "evaluation" / "037_02_lca_validation_checkpoint_summary.csv", index=False, encoding="utf-8-sig")
    by_ckpt = summarize_by_checkpoint(eval_df)
    by_ckpt.to_csv(OUT / "tables" / "037_02_lca_by_checkpoint_summary.csv", index=False, encoding="utf-8-sig")
    ...
    result = {
        ...
        "eval_summary": str((OUT / "evaluation" / "037_02_lca_validation_checkpoint_summary.csv").relative_to(ROOT)).replace("\\", "/"),
        "by_checkpoint": str((OUT / "tables" / "037_02_lca_by_checkpoint_summary.csv").relative_to(ROOT)).replace("\\", "/"),
    }
    (OUT / "037_02_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
