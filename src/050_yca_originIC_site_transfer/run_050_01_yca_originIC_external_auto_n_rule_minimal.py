"""050_01: YCA originIC minimal external auto-N baseline.
...
DEFAULT_CONFIG = ROOT / "configs" / "050_01_yca_originIC_external_auto_n_rule_nstd050_minimal.json"
...
def read_config(path: Path) -> dict[str, Any]:
    ...
    if cfg.get("station_code") != STATION or cfg.get("site") != SITE:
        raise ValueError("050_01 is registered for station_code=YCA and site=YC.")
    ...
    if present:
        raise ValueError(f"050_01 minimal auto-N must not include removed constraints: {present}")
    return cfg
...
def selected_rows(years: list[int]) -> pd.DataFrame:
    ...
    if sorted(out["year"].astype(int).tolist()) != sorted(years):
        raise RuntimeError("Some YCA validation years are missing from the split registry.")
...
def evaluate_year(run_config: dict[str, Any], env_config: dict[str, Any], year: int, rule: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any], dict[str, Any]]:
    env, env_args, rendered_text = make_env_auto_irrigation_external_n(env_config, year, f"YCA_{year}_050_01_external_auto_n")
    ...
    try:
        ...
        if daily.empty or not bool(daily["done"].iloc[-1]):
            raise RuntimeError(f"YCA{year} did not finish within {MAX_STEPS} steps")
...
def write_record(out: Path, cfg: dict[str, Any], summary: pd.DataFrame, failures: pd.DataFrame, elapsed: float) -> Path:
    ...
    lines = [
        f"# {cfg['task_id']} YCA originIC minimal external auto-N record",
        "",
        ...
        f"- Summary CSV: `{rel(out / 'evaluation' / '050_01_external_auto_n_summary.csv')}`.",
    ]
...
def run(cfg_path: Path, dry_run: bool, years_override: list[int] | None = None, run_id: str = "") -> dict[str, Any]:
    ...
    pf = {
        "task": "050_01_yca_external_auto_n_rule_minimal",
        ...
        "output_root": rel(out),
        "next_step_allowed": input_root.exists() and (input_root / "YC" / "CNYC0801.MZX").exists(),
    }
    if dry_run:
        return {"mode": "dry_run", **pf}
    if not pf["next_step_allowed"]:
        raise RuntimeError("preflight failed: YCA input root or source MZX missing")
    ...
    direct_ppo.write_yaml(env_config, out / "configs" / "050_01_resolved_env_config.yaml")
    ...
    try:
        for year in years:
            print(f"[050_01] YCA{year} {SCENARIO}", flush=True)
            try:
    ...
    summary.to_csv(out / "evaluation" / "050_01_external_auto_n_summary.csv", index=False, encoding="utf-8-sig")
    daily.to_csv(out / "evaluation" / "050_01_external_auto_n_daily.csv", index=False, encoding="utf-8-sig")
    rendered_df.to_csv(out / "evaluation" / "050_01_rendered_management_mode_audit.csv", index=False, encoding="utf-8-sig")
    failures.to_csv(out / "evaluation" / "050_01_failures.csv", index=False, encoding="utf-8-sig")
    ...
    (out / "050_01_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    return result
