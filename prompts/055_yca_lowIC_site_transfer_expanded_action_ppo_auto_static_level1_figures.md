# 055 YCA/YC lowIC site-transfer experiment prompt
...
1. `055_02`: regenerate four non-PPO baselines for `YCA` / `YC` with static management rows rendered as management level `1`.
2. `055_00`: train MaskablePPO using the frozen 046_10 method.
3. `055_01`: evaluate DSSAT native automatic irrigation plus minimal external auto-N.
4. `055_03`: draw yearly five-scenario daily plots and season-level bar charts from the corrected baseline, auto, and PPO outputs.
...
Do not use old `034_00` baseline outputs for final 055 five-scenario figures unless explicitly requested for diagnosis.
