...
WINDOWS_RUN_DIR = PROJECT_ROOT / "DSSAT_auto_validation" / "HLA_2004/run_CNFQ0802_DSSAT480_standalone/run_CNFQ0802_DSSAT480_standalone"
OUT_DIR = WINDOWS_RUN_DIR / "analysis_windows480_vs_pdi_yearly_ic0"
YEARS = list(range(2007, 2024))
FQZ_WEATHER_INPUT_DIR = PROJECT_ROOT / "DSSAT_auto_validation" / "FQZ_weather_inputs"
...
def render_single_year_mzx(year: int, out_path: Path) -> None:
    yy = year % 100
    yy2 = f"{yy:02d}"
    text = (WINDOWS_RUN_DIR / "CNFQ0801.MZX").read_text(encoding="latin1", errors="ignore")
    # Keep the file structure intact but make treatment 1 a one-year run.
    replacements = {
        "Sim2007": f"Sim{year}",
        "CNFQ2007": f"CNFQ{year}",
        "CNFQ0801": f"CNFQ{yy2}01",
        " 1 07162 07167": f" 1 {yy2}162 {yy2}167",
        " 1 07162 IR003": f" 1 {yy2}162 IR003",
        " 1 07202 FE006": f" 1 {yy2}202 FE006",
        " 1 07152   -99": f" 1 {yy2}152   -99",
        " 1 GE             23     1     S 07152": f" 1 GE              1     1     S {yy2}152",
        " 1 PL          00001 00001": f" 1 PL          {yy2}001 {yy2}001",
    }
...
def run_pdi_for_year(year: int, timeout_s: int = 90) -> dict[str, Any]:
    ...
    filex = input_dir / f"CNFQ{year % 100:02d}01.MZX"
    render_single_year_mzx(year, filex)

    aux: list[str] = []
    weather_src = WINDOWS_RUN_DIR / f"CNFQ{year % 100:02d}01.WTH"
    if not weather_src.exists():
        weather_src = FQZ_WEATHER_INPUT_DIR / f"CNFQ{year % 100:02d}01.WTH"
    cli_src = PROJECT_ROOT / "my_data" / "CNFQ.CLI"
    if not cli_src.exists():
        cli_src = FQZ_WEATHER_INPUT_DIR / "CNFQ.CLI"
    for src in [
        weather_src,
        PROJECT_ROOT / "my_data" / "FQ.SOL",
...
def plot_year(merged: pd.DataFrame, year: int, out_path: Path) -> None:
    ...
    fig.suptitle(f"FQZ {year} IC=0 null: Windows DSSAT 4.8.0 vs PDI yearly run", y=0.995)
...
def main() -> None:
    ...
    if not merged.empty:
        for year in sorted(merged["year"].dropna().astype(int).unique().tolist()):
            if year in YEARS:
                plot_year(merged, year, figures_dir / f"FQZ_IC0_windows480_vs_pdi_yearly_{year}.png")
        summary_rows = []
