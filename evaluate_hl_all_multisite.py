...
def parse_args():
    ...
    parser.add_argument('--all-amir-no-stress-cost', type=float, default=0.0)
    parser.add_argument('--all-water-stress-threshold', type=float, default=0.05)
    ...
    parser.add_argument('--site', default='HL', choices=['HL', 'SY', 'YC', 'LC', 'FQ'])
    parser.add_argument('--data-dir', default='./my_data')
    parser.add_argument('--prefer-suffix', default=None)
...
def set_reward_env(args):
    ...
    os.environ['GYM_DSSAT_ALL_AMIR_NO_STRESS_COST'] = str(args.all_amir_no_stress_cost)
    os.environ['GYM_DSSAT_ALL_WATER_STRESS_THRESHOLD'] = str(args.all_water_stress_threshold)
        ...
        for ep in range(n_episodes):
            ...
            while not done:
                ...
                record = {
                    ...
                    'swfac': latest_obs.get('swfac', obs_dict.get('swfac', np.nan)),
                    'nstres': latest_obs.get('nstres', obs_dict.get('nstres', np.nan)),
                    'topwt': latest_obs.get('topwt', obs_dict.get('topwt', np.nan)),
                    'grnwt': latest_obs.get('grnwt', obs_dict.get('grnwt', np.nan)),
                    'xlai': latest_obs.get('xlai', obs_dict.get('xlai', np.nan)),
                    'totir': latest_obs.get('totir', obs_dict.get('totir', np.nan)),
                    'rain': latest_obs.get('rain', obs_dict.get('rain', np.nan)),
                    'sw': latest_obs.get('sw', obs_dict.get('sw', np.nan)),
                    'runoff': latest_obs.get('runoff', obs_dict.get('runoff', np.nan)),
                    'ep': latest_obs.get('ep', obs_dict.get('ep', np.nan)),
                    'trnu': trnu,
                    'reward': reward,
                }
...
if __name__ == '__main__':
    ...
    print(f'## all stress cost: amir_no_stress={args.all_amir_no_stress_cost}, '
