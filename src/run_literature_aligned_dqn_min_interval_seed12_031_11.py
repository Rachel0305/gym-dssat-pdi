...
OUT = ROOT / "benchmark_results" / "031_11_literature_aligned_dqn_min_interval_seed12"
DOC = ROOT / "docs" / "031_11_literature_aligned_dqn_min_interval_seed12_record.md"
...
def make_sy2014_selection() -> pd.DataFrame:
    ...
    row["selection_reason"] = "031_11_literature_aligned_dqn_min_interval_seed12"
    return row
...
def train_seed(config: dict, env_config: dict, seed: int, year: int) -> dict[str, Any]:
    ...
    cfg["seed"] = int(seed)
    station = "SYA"
    model_path = OUT / "models" / station / f"literature_aligned_dqn_seed{seed}.zip"
    status = "ok"
    notes = ""
    env = None
    try:
        env = lit.make_env(cfg, env_config, station, year, seed, f"{station}_{year}_031_11_lit_dqn_seed{seed}_train", evaluation=False)
        model = DQN(
            "MlpPolicy",
            env,
            verbose=1,
...
def evaluate_seed(config: dict, env_config: dict, seed: int, year: int, model_path: Path, split: str) -> dict[str, Any]:
    from stable_baselines3 import DQN
    ...
    env = lit.make_env(cfg, env_config, station, year, seed, f"{station}_{year}_031_11_lit_dqn_seed{seed}_{split}", evaluation=True)
    ...
    daily_path = daily_dir / f"{year}_seed{seed}_{split}_literature_aligned_dqn_daily.csv"
    ...
    clipped_high_n_requests = int(((pd.to_numeric(daily.get("raw_action_anfer", pd.Series(dtype=float)), errors="coerce") >= 120) & (n == 0)).sum()) if len(daily) else 0
    return {
        "algorithm": "literature_aligned_DQN",
        "policy_name": "lit_dqn_5k_interval7",
        "station_code": station,
        "year": int(year),
        "seed": int(seed),
        "split": split,
        ...
        "clipped_high_n_request_days": clipped_high_n_requests,
...
def main() -> None:
    ...
    selection_path = OUT / "configs" / "031_11_sy2014_selection.csv"
    ...
    env_config_path = OUT / "configs" / "031_11_resolved_env_config.yaml"
    ...
    for seed in SEEDS:
        ...
        if train_row["run_status"] == "ok":
            model_path = ROOT / train_row["model_path"]
            eval_rows.append(evaluate_seed(config, env_config, seed, year, model_path, "eval"))
        else:
            eval_rows.append({"algorithm": "literature_aligned_DQN", "seed": seed, "split": "eval", "run_status": "failed", "notes": train_row["notes"]})
    ...
    eval_path = OUT / "evaluation" / "literature_aligned_dqn_seed12_eval_summary.csv"
    ...
    seed0_path = ROOT / "benchmark_results" / "031_10_literature_aligned_dqn_min_interval_smoke" / "evaluation" / "literature_aligned_dqn_eval_summary.csv"
    comparison = eval_summary.copy()
    comparison["source"] = "031_11"
    if seed0_path.exists():
        seed0 = pd.read_csv(seed0_path)
        seed0 = seed0[seed0["split"].eq("eval")].copy()
        seed0["seed"] = 0
        seed0["source"] = "031_10"
        comparison = pd.concat([seed0, comparison], ignore_index=True, sort=False)
    comparison_path = OUT / "031_11_seed012_comparison.csv"
    comparison.to_csv(comparison_path, index=False, encoding="utf-8-sig")

    result = {
        "task": "031_11_literature_aligned_dqn_min_interval_seed12",
        "training_run": True,
        "site_year": "SYA2014",
        "seeds": SEEDS,
        "timesteps_each": int(config["total_timesteps"]),
        "frozen_config_source": str(BASE_CONFIG.relative_to(ROOT)),
    ...
    (OUT / "031_11_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False, default=str), encoding="utf-8")
